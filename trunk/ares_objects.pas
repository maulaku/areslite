...
type FSTSessionState = (SessIdle,
 ...
 // Cache ¼­¹ö Record
  tcacheServer = class(Tobject)
    reports, attempts, connects,
      first_seen, last_seen, last_attempt: cardinal;
    inUse, dejaVu: boolean;
    ip: cardinal;
    port: word;
    constructor create;
    destructor destroy; override;
...
uses
  ...
  helper_ares_cacheservers, ares_types, ufrmmain, helper_unicode,
...
constructor tcacheserver.create;
begin
  last_attempt := 0;
  first_seen := 0;
  last_seen := 0;
...
function tcacheserver.rate: single;
begin
  ...
  result := (connects * 50.0) +
    ...
    (((last_seen - helper_ares_cacheservers.db_cacheservers_oldest_last_seen) / 600) * 1.0);
end;
