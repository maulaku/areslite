...
uses
  ...
  types_cacheserver,const_cacheserver,const_ares,helper_supernode_crypt,hashlist,
  helper_diskio;
...
tthread_supernode = class(TThread)
  ...
  UnreliableSupernodes:tmylist;
   ...
   // throttle_udp:cardinal; //lo aggiorniamo in base a orizzonte medio remoto
    ...
    procedure broadcast_caches_urls(cont:string; is_new:boolean);//max 396 chars
    procedure check_cache_patch;
    procedure cachequery_deal;
   ...
   // function comp_search_kewds(list:tnapcmdlist;kw:pkeyword):boolean;
    ...
    procedure parse_received_cache;
...
begin
 if tim-last_Halfsecond<500 then exit;  //mezzo secondo in realtà?
   ...
   try
    cachequery_deal; //chiede links and stats (ottiane links possibili)
    checksync;
   except
   end;
...
procedure tthread_supernode.big_dump; //DEBUG
var
i,h:integer;
us:tlocaluser;
...
procedure tthread_supernode.cachequery_deal;
              procedure FreeCache(freesocket:boolean=true);
               begin
                   if freesocket then TCPSocket_Free(socket_cache);
                   stato_supernodecache_query:=STATO_SUPERNODE_CACHE_QUERY_IDLE;
                   cachequery_last:=0; // riproviamo subito
               end;
var
er,len,previous_len:integer;
to_recv:word;
ip:string;
str:string;
his_unenc_key,his_enc_key:string;
...
case stato_supernodecache_query of

 STATO_SUPERNODE_CACHE_QUERY_IDLE:begin

        if ((tim-cachequery_last>10*MINUTE) or            //comportamento 'speciale' nei primi 20 minuti
             ((tim-cachequery_last>2*MINUTE) and (tim-start_time<20*MINUTE))) then begin //se sono sotto vado ogni 4 minuti a chiedere

                   ip:=cache_get_1host;  //proviamo con bassa priorità!
                   if ip='' then exit;//fail magari per timeout  prevent flooding root
                  ip_cache_cardinal:=inet_addr(pchar(ip)); //globale per inclusione anche suo indirizzo in lista broadcasted to client
                  port_cache:=get_ppca(ip_cache_cardinal);

                 received_cache_str:='';

                 stato_supernodecache_query:=STATO_SUPERNODE_CACHE_QUERY_CONNECTING;

                   //TCPSocket_Free(socket_cache); //free old!
                  socket_cache:=TCPSocket_Create;
                   TCPSocket_Block(socket_cache,false);

                  TCPSocket_Connect(socket_cache, ip, inttostr(port_cache),er);
                  if ((er<>0) and (er<>WSAEWOULDBLOCK)) then begin
                   helper_ares_cacheservers.Cache_putDisConnected(ip_cache_cardinal);
                   FreeCache;
                   exit;
                  end;
                  cachequery_last:=gettickcount;
             end; //80 servers da provare ogni 10 minuti?

                                  end;

 STATO_SUPERNODE_CACHE_QUERY_CONNECTING:begin
                         if tim-cachequery_last>TIMOUT_SOCKET_CONNECTION then begin
                           helper_ares_cacheservers.Cache_putDisConnected(ip_cache_cardinal);
                           FreeCache;
                           exit;
                         end;

                         if not TCPSocket_CanWrite(socket_cache,0,er) then begin
                          if ((er<>0) and (er<>WSAEWOULDBLOCK)) then begin
                           helper_ares_cacheservers.Cache_putDisConnected(ip_cache_cardinal);
                           FreeCache;
                          end;
                          exit;
                             ...
                             str:=int_2_word_string(0)+chr(MSG_CCACHE_SUPPRELOGIN);
                             TCPSocket_SendBuffer(socket_cache,@str[1],3,er);
                             ...
                             cachequery_last:=tim;
                             stato_supernodecache_query:=STATO_SUPERNODE_CACHE_QUERY_WAITING_FORKEY;
                             received_cache_str:=''; //azzeriamo non si sa mai
                         end;

 STATO_SUPERNODE_CACHE_QUERY_WAITING_FORKEY:begin
                          if tim-cachequery_last>(TIMOUT_SOCKET_CONNECTION*2) then begin //facciamo rispondere prima loro...eventualmente con (sei firewalled)
                           helper_ares_cacheservers.Cache_putDisConnected(ip_cache_cardinal);
                           FreeCache;
                           exit;
                         end;
                         if not TCPSocket_CanRead(socket_cache,0,er) then begin
                            if ((er<>0) and (er<>WSAEWOULDBLOCK)) then begin
                             helper_ares_cacheservers.Cache_putDisConnected(ip_cache_cardinal);
                             FreeCache;
                            end;
                          ...
                          previous_len:=length(received_cache_str);

                           if previous_len<3 then len:=TCPSocket_RecvBuffer(socket_cache,@buffer_ricezione,3-previous_len,er) else begin
                             move(received_cache_str[1],to_recv,2);
                             if to_recv>1024 then begin
                               helper_ares_cacheservers.Cache_putDisConnected(ip_cache_cardinal);
                               FreeCache;
                              exit;
                             end;
                             len:=TCPSocket_RecvBuffer(socket_cache,@buffer_ricezione,(to_recv-previous_len)+3,er)
                           end;
                           ...
                           if previous_len<3 then begin
                             setlength(received_cache_str,previous_len+len);
                             move(buffer_ricezione,received_cache_str[previous_len+1],len);
                            exit;
                           end;
                            ...
                            if length(received_cache_str)<to_recv+3 then exit;//ancora non ho tutto
                            if length(received_cache_str)<131 then begin
                              helper_ares_cacheservers.Cache_putDisConnected(ip_cache_cardinal);
                              FreeCache;
                             exit;
                            end;
                            if received_cache_Str[3]<>chr(MSG_CACHE_HERE_MY_KEY_TO_SUP) then begin
                             helper_ares_cacheservers.Cache_putDisConnected(ip_cache_cardinal);
                             FreeCache;
                             exit;
                            end;

                             delete(received_cache_str,1,3);
                            his_unenc_key:=d3a(received_cache_Str,port_cache);
                             delete(his_unenc_key,129,length(his_unenc_key));//nel caso fosse più lunga
                            his_enc_key:=get_crypt_cache_key(his_unenc_key);


                             cachequery_last:=tim;
                             stato_supernodecache_query:=STATO_SUPERNODE_CACHE_QUERY_FLUSHING_STATS;
                             received_cache_str:=''; //azzeriamo non si sa mai
                            cache_str_toflush:=his_enc_key+
                                               e64(sup_unencrypted_login_key+    //128
                                                   int_2_word_string(my_tcp_port)+
                                                   int_2_word_string(LinkedSupernodes.count)+
                                                   int_2_word_string(user_list.count)+
                                                   int_2_word_string(my_buildnumber)+
                                                   int_2_dword_string(shared_count)+
                                                   int_2_dword_string(shared_count*8)+
                                                   chr(1),port_cache);
                                         cache_str_toflush:=int_2_word_string(length(cache_str_toflush))+
                                                            chr(MSG_CCACHE_HASH_ADDME_LATEST)+
                                                            cache_str_toflush;
 end;

 STATO_SUPERNODE_CACHE_QUERY_FLUSHING_STATS:begin
                          if tim-cachequery_last>(TIMOUT_SOCKET_CONNECTION*2) then begin //facciamo rispondere prima loro...eventualmente con (sei firewalled)
                           helper_ares_cacheservers.Cache_putDisConnected(ip_cache_cardinal);
                           FreeCache;
                           exit;
                         end;
                         if length(cache_str_toflush)=0 then begin
                           helper_ares_cacheservers.Cache_putDisConnected(ip_cache_cardinal);
                           FreeCache;
                           exit;
                         end;
                         TCPSocket_SendBuffer(socket_cache,@cache_str_toflush[1],length(cache_str_toflush),er);
                         if er=WSAEWOULDBLOCK then exit;
                          ...
                          if er<>0 then begin
                            ...
                            exit;
                             ...
                             cachequery_last:=tim;
                             stato_supernodecache_query:=STATO_SUPERNODE_CACHE_QUERY_RECEIVING;
                             received_cache_str:=''; //azzeriamo non si sa mai
                             cache_str_toflush:='';
 end;

 STATO_SUPERNODE_CACHE_QUERY_RECEIVING:begin
                         if tim-cachequery_last>(TIMOUT_SOCKET_CONNECTION*2) then begin //facciamo rispondere prima loro...eventualmente con (sei firewalled)
                           helper_ares_cacheservers.Cache_putDisConnected(ip_cache_cardinal);
                           FreeCache;
                           exit;
                          ...
                          exit;
                           ...
                           if previous_len<3 then begin
                            ...
                            if length(received_cache_str)<to_recv+3 then exit;//ancora non ho tutto
                               ...
                               TCPSocket_Free(socket_cache);
                              if length(received_cache_str)<20 then begin
                               helper_ares_cacheservers.Cache_putDisConnected(ip_cache_cardinal);
                               FreeCache(false);
                               exit;
                              end;  //non ho neanche header!!
                              if received_cache_str[3]<>chr(MSG_CCACHE_HERE_ALLSERV) then begin
                               helper_ares_cacheservers.Cache_putDisConnected(ip_cache_cardinal);
                               FreeCache(false);
                               exit;
                              end;
                              delete(received_cache_Str,1,3);
                              received_cache_str:=d64(received_cache_str,port_cache);
                              
                              try
                              parse_received_cache;  //finally we did it
                              except
                              end;


                            stato_supernodecache_query:=STATO_SUPERNODE_CACHE_QUERY_IDLE; //readi for next

                            helper_ares_cacheservers.Cache_putHasConnected(ip_cache_cardinal);
...
procedure tthread_supernode.parse_received_cache;
var
str,ipsd:string;
tipo:byte;
lung:word;
//is_ares:boolean;
added_supernodes:integer;
begin
try
...
if not comparemem(@sup_enc_keyto_cache[1],@received_cache_str[1],20) then begin
 exit;
end;

  delete(received_cache_str,1,20);

  str:=received_cache_str;
 ...
 //is_ares:=False;
  ...
  while (length(str)>2) do begin
   ...
   case tipo of
    1:begin
      if str[1]=chr(10) then begin  //special supernode....  can't be behind lan!
        if str[2]=CHRNULL then begin //spegniti!
         //terminate; //2965 tolto spegniti

         exit;
        end else
        if str[2]=chr(1) then begin //my ip non rigirato, rigiro io
         mylocalip_dword:=copy(str,3,4);
         locip:=chars_2_dword(reverse_order(copy(str,3,4)));   //questo sarà compito di cache server dinamico in futuro...
           ipsd:=int_2_dword_string(locip);
           in1_decrypt:=chars_2_word(copy(ipsd,1,2)); //1
           in2_decrypt:=chars_2_word(copy(ipsd,3,2)); //2
        end;
      end else begin
         addAvSupernode(chars_2_dword(copy(str,1,4)),chars_2_word(copy(str,5,2)));
          inc(added_supernodes);
      end;
      delete(str,1,6); //togliamo blocco
    end;
    21:begin
       // globale_cache_stat_string:=copy(str,1,12); //user-files-giga overall
        delete(str,1,12);
    end;
    25:begin
        try
          WANTED_USER_IN_CLUSTER:=chars_2_word(copy(str,1,2)); // 80 utenti, finche non arrivo a quella quota continuo a linkare
           if WANTED_USER_IN_CLUSTER<5000 then WANTED_USER_IN_CLUSTER:=9000;
           if WANTED_USER_IN_CLUSTER>30000 then WANTED_USER_IN_CLUSTER:=9000;
          MAX_LINKED_HSERVERS:=ord(str[3]);
           if MAX_LINKED_HSERVERS<10 then MAX_LINKED_HSERVERS:=90;
           if MAX_LINKED_HSERVERS>120 then MAX_LINKED_HSERVERS:=90;
         delete(str,1,3);
        except
        end;
    end;
    26:begin
       //throttle_udp:=chars_2_word(copy(str,1,2));
       //if throttle_udp>20000 then throttle_udp:=333 else
       //if throttle_udp<10 then throttle_udp:=333;
       delete(str,1,2);
    end;
    30:begin      //il 30 viene mandato da cache 2940 che mandano a 6 per 6, ignoriamo per ora...
       lung:=chars_2_word(copy(str,1,2));
       delete(str,1,lung+2);
       end;
    31:begin    //da 2941 mandano a 4 in 4...facciamo broadcast più efficiente
    //NOTE!!!  con l'arrivo della versione cache > 2952 disattivare questo comando
         lung:=chars_2_word(copy(str,1,2));
         if length(str)>=lung+2 then begin  //ho tutto? nuovo messaggio con host da solo 4 bytes, il messaggio 30 è stato errore
          // if lung>396 then broadcast_caches_urls(int_2_dword_string(ip_cache_cardinal)+
          //                                        copy(str,3,396),false)
          //  else broadcast_caches_urls(int_2_dword_string(ip_cache_cardinal)+
          //                             copy(str,3,lung),false); //inclusione anche di lui stesso come buono
           delete(str,1,lung+2);
         end else break;
     end;
     34:begin
     ...
     35:begin    //da 2952+
         lung:=chars_2_word(copy(str,1,2));
         if lung>4 then begin
          if length(str)>=lung+2 then begin  //ho tutto? nuovo messaggio con host da solo 4 bytes, il messaggio 30 è stato errore
            if lung>396 then broadcast_caches_urls(int_2_dword_string(ip_cache_cardinal)+
                                                  copy(str,3,396),true)
             else broadcast_caches_urls(int_2_dword_string(ip_cache_cardinal)+
                                       copy(str,3,lung),true); //inclusione anche di lui stesso come buono
            delete(str,1,lung+2);
          end else break;
         end;
     end;
     ...
     36:begin  //2960+
         lung:=chars_2_word(copy(str,1,2));
         if lung>3 then begin
          if length(str)>=lung+2 then begin  //ho tutto? nuovo messaggio con host da solo 4 bytes, il messaggio 30 è stato errore
           // if lung>396 then broadcast_Chatcaches_urls(copy(str,3,396))
            // else broadcast_Chatcaches_urls(copy(str,3,lung)); //inclusione anche di lui stesso come buono
            delete(str,1,lung+2);
          end else break;
         end;
     end
       ...
       lung:=chars_2_word(copy(str,1,2)); //prox messaggi contengono sempre len per futura compatibilità 2951+
       delete(str,1,lung+2);
     end;
...
procedure tthread_supernode.broadcast_caches_urls(cont:string; is_new:boolean);//max 396 chars
 function e12e(const S: String; b: Word): String;
 var
 I: cardinal;
 begin
...
b:=a1(my_sc,byte1_ransend,ff[my_ca]);
   ...
   if is_new then cont:=e12e(cont,1432);

     setlength(str,lenpkt);  //allocazione

       move(lencon,str[1],2);
       if is_new then
        str[3]:=chr(MSG_SERVER_HERE_CACHEPATCH3) //2952
       else
        str[3]:=chr(MSG_SERVER_HERE_CACHEPATCH2);

         str[4]:=byte1_ransendchr[1];
         str[5]:=byte1_ransendchr2[1];
         for i:= 1 to Lento do begin  //criptiamo direttamente su buffer
           str[i+5] := char(byte(cont[i]) xor (b shr 8));
           b := (byte(str[i+5]) + b) * 52845 + 22719;
         end;
                              
  latest_cache_url_string:=str; //per invio non appena c'è login
...
procedure tthread_supernode.check_cache_patch;
var
i,er:integer;
us:tlocaluser;
begin
...
if length(latest_cache_url_string)=0 then exit;

 for i:=0 to user_list.count-1 do begin
  us:=user_list[i];
  if us.disconnect then continue;
  if tim-us.logtime<6*MINUTE then continue;

  if ((us.last_cache_patch=0) or
     (tim-us.last_cache_patch>HOUR)) then begin

       if us.encrypted_out then begin
         if TCPSocket_CanWrite(us.socket,0,er) then begin
          encrypt_buffer(@latest_cache_url_string[1],length(latest_cache_url_string),@buffer_ricezione[0],us.outKey);
          TCPSocket_SendBuffer(us.socket,@buffer_ricezione[0],length(latest_cache_url_string),er);
         end;
       end else TCPSocket_SendBuffer(us.socket,@latest_cache_url_string[1],length(latest_cache_url_string),er);
...
delete(content,1,pos(CHRNULL,content));
   ...
   if length(content)>1 then begin
    ...
    if length(GlobUser.agent)<2 then begin
     ...
     delete(content,1,pos(CHRNULL,content));
    if length(content)>=4 then GlobUser.his_local_ip:=copy(content,1,4) else
 ...
 while (posiz+7<len_keyword) do begin //parsiamo le keywords
   ...
   if posiz+6+lenkey>len_keyword then break;//keyword segata non includiamo!
    ...
    case buffer_parse_keywords[posiz] of
     ...
     4:pfield:=precord_field(rfield_category);
     5:pfield:=precord_field(rfield_language);
     6:pfield:=precord_field(rfield_date) else break;
    end;
