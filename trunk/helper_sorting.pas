...
function ordina_candidate_cache_peR_numtry(item1,item2:pointer):integer; //prima posizione inferiore
...
function sort_cacheservers_bestrating(item1,item2:pointer):integer;  //most rated first
function sort_cacheservers_worstrating(item1,item2:pointer):integer;  //worst rated first
...
function ordina_per_build(item1,item2:pointer):integer;
...
uses
types_supernode,types_cacheserver,helper_strings,helper_ares_nodes,btcore;
...
function ordina_per_build(item1,item2:pointer):integer;
 var
 sup1,sup2:precord_cache_hash_supernode;
begin
sup1:=precord_cache_hash_supernode(item1);
sup2:=precord_cache_hash_supernode(item2);
 result:=sup2^.build_no-sup1.build_no;
end;

function ordina_candidate_cache_peR_numtry(item1,item2:pointer):integer; //prima posizione più alta (quelli con più try)
   var
   item1p,item2p:precord_candidato_cache;
begin
   item1p:=precord_candidato_cache(item1);
   item2p:=precord_candidato_cache(item2);
    result:=item2p^.numtry-item1p^.numtry;
end;
...
function sort_cacheservers_bestrating(item1,item2:pointer):integer;  //best rated first
var
cache1,cache2:tcacheserver;
begin
 cache1:=item1;
 cache2:=item2;
 result:=round(cache2.rate)-round(cache1.rate);
end;
...
begin
 cache1:=item1;
 cache2:=item2;
 result:=round(cache1.rate)-round(cache2.rate);
end;
