...
function get_text_webpage(var res:byte; url:string; port:string = '80' ):string;
            ...
            if pos('HTTP/1.1 200 OK'+CRLF,ricevuto)<>1 then begin //wrong or redir
               site.free;
                 if pos(CRLF+'location:',lowercase(ricevuto))<>0 then begin
                  ...
                  location:=trim(location);
                     if pos('http://',lowercase(location))<>1 then location:='http://'+location; //aggiungiamo http se non c'è
                     if length(extract_document_from_url(location))<2 then location:=location+extract_document_from_url(url); //aggiungiamo documento se non c'è
                    result:=get_text_webpage(res,location,port);
                    exit;
                end;
              ...
              exit;
                ...
                if pos(CRLF+'content-length:',lowercase(ricevuto))>0 then begin
                   ...
                   sizi:=strtointdef(siz,-1);
                    ...
                    delete(ricevuto,1,pos(CRLF+CRLF,ricevuto)+3);
                     chunked:=false;
                     result:=ricevuto;
                       if length(result)>=sizi then begin //ho tutto yeppa!
                        site.free;
                        exit;
                       end;
                     yet_to_receive:=sizi;
                ...
                if ((pos(CRLF+'transfer-encoding: chunked'+CRLF,lowercase(ricevuto))<>0) or
                    ...
                    delete(ricevuto,1,pos(CRLF+CRLF,ricevuto)+3);
                      chunked:=true;
                      yet_to_receive:=-1;  //in seguito parso quanto....
                   result:='';//copiamo primo pezzo per ora....
                end;

        end;     //fine se non ho ancora double empty ln!
     end;


     
     ////////////////////begin parse body
     if ricevuto_header then begin  //sto ricevendo body!

            if not chunked then begin
              result:=result+ricevuto;//aggiungiamo roba appena ricevuta a risultato
              ricevuto:='';
             if length(result)>=yet_to_receive then begin //ho tutto yeppa!
               site.free;
               exit;
             end;
            end else begin   //se invece sono chunked....
                         ...
                         if yet_to_receive=-1 then begin
                             if pos(CRLF,ricevuto)>0 then begin
                              ...
                              yet_to_receive:=hextoint(chunksizestr);
                               ...
                               delete(ricevuto,1,pos(CRLF,ricevuto)+1); //togliamo indicazione chunk!
                              end else break;
                          end;
                          if yet_to_receive<>-1 then begin //se so quanto prendere ancora...
                             if length(ricevuto)>=yet_to_receive then begin  //se ho già preso abbastanza...
                                 result:=result+copy(ricevuto,1,yet_to_receive);  //copiamo in result!
                                   delete(ricevuto,1,yet_to_receive+2); //compreso CRLF
                                  yet_to_receive:=-1;
                             end else break;
                          end;

                      end;  //end while parse
           end;  //fine se è chunkeder

     end;   //end if ricevuto header....
