...
begin
  ...
  {$ifdef ThemeSupport}
    {$ifndef COMPILER_7_UP}
      ThemeServices := TThemeServices.Create(Application.Handle);
    {$endif COMPILER_7_UP}
  {$endif ThemeSupport}
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Tree is TCustomCometStringTree then
          with TCustomCometStringTree(Tree) do
...
begin
  ...
  if Result and (Msg.Message = WM_NCMOUSEMOVE) then
    ...
    // Work around problems with keypresses while doind hint animation.
    if (Msg.Message >= WM_KEYFIRST) and (Msg.Message <= WM_KEYLAST) and (tsInAnimation in FHintData.Tree.FStates) and
      HandleAllocated and IsWindowVisible(Handle) and TranslateMessage(Msg) then
      DispatchMessage(Msg);
end; }
...
//node:=GetNodeAt(punto1.X, punto1.Y);
GetHitTestInfoAt(punto1.x,punto1.y,true,hitinfo);
 if hitinfo.hitnode=nil then exit;
 if not (hiOnItemLabel in HitInfo.HitPositions) then exit;

if assigned(FOnHintStart) then FOnHintStart(self,hitinfo.hitnode);
