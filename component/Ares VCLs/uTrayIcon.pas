...
uses
  ...
  Menus, {ShellApi,} ExtCtrls,ComCtrls;
...
begin
  ...
  begin
   ...
   {   WM_CLOSE,WM_DESTROY,WM_NCDESTROY,WM_QUIT : begin
     // mes/sagebox(0,pchar('tray:'+inttostr(msg.msg)),pchar('gf'),mb_ok);
        Msg.Result := 1;
      end;


      WM_QUERYENDSESSION,WM_ENDSESSION: begin
      //  mess/agebox(0,pchar('tray:'+inttostr(msg.msg)),pchar('gf'),mb_ok);
        //if cool.Fhandle_main<>0 then postmessage(cool.FHandle_main,(WM_USER+152),0,0);//quit
       //try
       // if assigned(cool.OnEndSession) then cool.OnEndSession(cool);
       //except
      // end;
        if cool.Fhandle_main<>0 then sendmessage(cool.FHandle_main,(WM_USER+152),0,0);
        Msg.Result := 1;
      end;  }
...
begin
  if Assigned(TrayIconHandler) then begin
    TrayIconHandler.Remove;
    if TrayIconHandler.RefCount = 0 then begin
      // Destroy handler
...
begin
  Result := False;
  ok := True;
  if (csDesigning in ComponentState) then ok := ((SettingPreview) or (FDesignPreview));
  ...
  if ok then begin
    ...
    if (FHint <> '') and (FShowHint) then begin
     //if length(Fhint)>63 then delete(FHint,64,length(FHint));
      ...
      str:=FHint;
      StrLCopy(IconDataA.szTip, PChar(String(str)), SizeOf(IconDataA.szTip)-1);
      //Fhint:=strpas(str);
...
{function TTrayIcon.GetTooltipHandle: HWND;
  ...
  while wnd <> 0 do begin
    ...
    if pidTaskBar = pidWnd then
        Get the tooltip style. The tooltip for tray icons does not have the
         TTS_NOPREFIX style.
      if (GetWindowLong(wnd, GWL_STYLE) and TTS_NOPREFIX) = 0 then Break;
