...
type
  ...
  procedure ShouldupdateClientHashes;
  ...
  procedure CheckDownloadsStatus;
    ...
    procedure add_downloads_recursive(const folder:widestring);
...
procedure tthread_download.add_downloads_recursive(const folder:widestring);
var
dirinfo:tsearchrecW;
doserror:integer;
download:tdownload;
...
doserror:=helper_diskio.FindFirstW(folder+'\___ARESTRA___*.*',faanyfile,dirinfo);
 ...
 DosError := helper_diskio.FindFirstW(folder+'\*.*', faDirectory, dirinfo);
      ...
      if terminated then exit;
       if (((dirinfo.attr and faDirectory) = 0) or
         ...
         add_downloads_recursive(dira+'\'+dirinfo.name); {Time for the recursion!}
...
procedure tthread_download.ShouldupdateClientHashes;
begin
vars_global.changed_download_hashes:=true;
end;
...
add_downloads_recursive(vars_global.myshared_folder);
synchronize(ShouldupdateClientHashes);
...
if download_globale.lista_risorse.count>0 then
  if loc_outgoing_connections<MAX_OUTCONNECTIONS then ActivateSources(download_globale,num_max_sources);
...
begin
 ...
 if loc_is_connecting then if tryn>2 then exit;
...
with risorsa do begin
   ...
   randoms:=inttohex(random(256),2)+
     ...
     if vars_global.im_firewalled then begin // UDP Push
      ...
      risorsa.UDP_socket:=synsock.socket(PF_INET,0,IPPROTO_UDP);
        ...
        if download.FPieceSize>0 then
         ...
         case HTTPResultCase of
           ...
           HTTPOK:begin
                   ...
                   if BeginDownload(risorsa,download,HTTPBody)=0 then inc(risorsa.succesfull_factor,10);
           end;
