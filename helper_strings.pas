...
uses
  sysutils, classes2, const_ares, synsock;
...
uses
  helper_urls, helper_unicode, ares_types, umediar;
...
begin
  ...
  if ((length(title) > 0) and (length(artist) > 0) and (length(album) > 0)) then result := artist + ' - ' + album + ' - ' + title
  else
    if ((length(title) > 0) and (length(artist) > 0)) then result := artist + ' - ' + title;
...
var
  i, rnum: integer;
  c: char;
begin //ammetiamo virgoletta, + e quadre
  result := '';
